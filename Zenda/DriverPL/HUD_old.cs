...
namespace Zenda.DriverPL
{
	public class HUDElement_old {
        ...
        // Other
        private HUD_old _hud;

        /// <summary>
        /// 
        /// </summary>
        /// <returns></returns>
        public HUD_old GetHUD() {
        	return _hud;
        }
	    // Useful function (took from HUDElement_old class)
	    /// <summary>
		...
		public HUDElement_old(byte t,byte pad,short grp,short input,short id,float x,float y,float w,float h,float sx,float sy,float r,float g,float b,float a) {
		    this.Type = t;
		...
		public HUDElement_old(HUD_old parenthud,byte t,byte pad,short grp,short input,short id,float x,float y,float w,float h,float sx,float sy,float r,float g,float b,float a) {
		    _hud = parenthud;
	    	this.Type = t;
		    Pad = pad; Group = grp; Input = input; Id = id; 
		    X = x; Y = y;
	    ...
	    public HUDElement_old(byte[] buffer) {
	    	fromBinary(buffer);
	    }
	    // Same as code above but indexing the HUD x2
	    public HUDElement_old(HUD_old parenthud,byte[] buffer) {
	    	_hud = parenthud;
	    	fromBinary(buffer);
	    }
	    ...
	    public byte[] ToBinary() {
	    	// Work in progress
	    	int HUDElement_oldBufferSize = 112; // just in case the HUD doesn't exist
	    	if (_hud!=null) {
	    	   HUDElement_oldBufferSize = _hud.HUDElement_oldBufferSize; // alright, the HUD exists. get the buffer size already!
	    	}
	    	byte[] buffer = new byte[HUDElement_oldBufferSize];
	    	// bytes
	    	buffer[0] = Type;
	    	buffer[1] = Pad;
	...
	{
		// For HUD elements
		public readonly int HUDElement_oldBufferSize = 112;
		...
		public HUDElement_old[] Elements;
		...
		private void fromBinary(byte[] buffer) {
            ...
            Elements = new HUDElement_old[Count]; // index
            // binary > HUDElement_old
            ...
            for (int id=0;id<Count;id++) {
            	...
            	#endif
            	Elements[id] = new HUDElement_old( this, byteSub(buffer,off,HUDElement_oldBufferSize) );
            	off = off+HUDElement_oldBufferSize;
            }
            #if DEBUG
		...
		public HUD_old(string filename)
		{
		...
		public HUD_old(FileStream file)
		{
			if (file!=null) {
				fromBinary(file.ReadAllBytes());
			}			
